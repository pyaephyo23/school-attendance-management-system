...
//				+ "AND takes.students_id = ? " + "AND semester.semester_name = ? " + "GROUP BY course_name";
		...
		String sql = "SELECT name, COUNT(attendance.id) AS total_attendance, SUM(CASE WHEN attendance.is_present = 1 THEN 1 ELSE 0 END) AS current_attendance, code, code, courses.course_name, semester.semester_name FROM attendance LEFT JOIN students ON attendance.students_id = students.id LEFT JOIN account ON students.id = account.id LEFT JOIN timeslot ON attendance.timeslot_id = timeslot.id LEFT JOIN courses ON timeslot.courses_id = courses.id LEFT JOIN semester ON timeslot.semester_id = semester.id WHERE attendance.date BETWEEN ? AND ?  AND students.id = ? AND semester.semester_name = ?  GROUP BY courses.course_name;";
